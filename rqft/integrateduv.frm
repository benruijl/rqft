...
S eulergamma, log4pi, pi, cl2, sqrt3;
CF uvid;
...
#procedure TruncateExpansion(SPURIOUSPOWER)
    argument rat;
        id ep^{`MAXPOLE'+`SPURIOUSPOWER'+1} = 0;
    endargument;
#endprocedure
...
#procedure Masters()
    ...
    id uvid(2,2) = -4/9*sqrt3*cl2 + alarmt2m22 * rat(ep, 1);

* mercedes
    id uvid(3,1,1,1,1,1,1,1) = 2 * z3*rat(1,ep) + alarmt3m111111;
* 5-edge
    id uvid(3,1,0,1,1,1,1,1) = mUV^2*(rat(1,ep^3) + rat(17,3*ep^2) + (67/3 - 4*sqrt3*cl2 + pi^2/4) * rat(1,ep) + alarmt3m011111);
* banana
    id uvid(3,1,0,1,1,1,0,1) = mUV^4*(rat(2,ep^3) + rat(23,3 * ep^2) + (35 + pi^2)*rat(1,2 * ep) + 1/12 * (275 + 23 * pi^2 - 24 * z3) + alarmt3m011101*rat(ep,1));
* sunrise-bubble
    id uvid(3,1,0,0,1,1,1,1) = mUV^4*(rat(3,2 * ep^3)+ rat(6,ep^2) + (-16*sqrt3*cl2 + 3*(44 + pi^2))*rat(1,8 * ep) + alarmt3m001111);
* triple-bubble
...
#procedure SubstituteMasters()
    ...
    B+ uvid;
    .sort:masters-1;
    PolyRatFun rat(expand,ep,{`MAXPOLE'+`SPURIOUSPOLE'});
    Keep brackets;
    ...
    .sort:truncation;
    PolyRatFun rat(expand,ep,`MAXPOLE');
